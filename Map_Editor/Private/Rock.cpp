...
HRESULT CRock::Initialize(void* pArg)
{
	...
	const _tchar* pModelComponentTag = static_cast<const _tchar*>(pArg);

	if (FAILED(Ready_Components(pModelComponentTag)))
		return E_FAIL;
...
HRESULT CRock::Ready_Components(const _tchar* pModelComponentTag)
{
	/* Com_Model */
	if (FAILED(__super::Add_Component(ENUM_CLASS(LEVEL::VILLAGE), pModelComponentTag,
...
CStaticMap* CRock::Clone(void* pArg)
{

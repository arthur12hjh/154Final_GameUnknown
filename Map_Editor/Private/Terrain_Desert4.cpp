#include "pch.h"
#include "Terrain_Desert4.h"
#include "GameInstance.h"

CTerrain_Desert4::CTerrain_Desert4(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject{ pDevice, pContext }
{
}

CTerrain_Desert4::CTerrain_Desert4(const CTerrain_Desert4& Prototype)
	: CGameObject{ Prototype }
{
}

HRESULT CTerrain_Desert4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTerrain_Desert4::Initialize(void* pArg)
{
...
void CTerrain_Desert4::Priority_Update(_float fTimeDelta)
{
	m_pNavigationCom->Update(XMLoadFloat4x4(m_pTransformCom->Get_WorldMatrixPtr()));
}

void CTerrain_Desert4::Update(_float fTimeDelta)
{
	//m_pVIBufferCom->Culling(XMLoadFloat4x4(m_pTransformCom->Get_WorldMatrixPtr()));
}

void CTerrain_Desert4::Late_Update(_float fTimeDelta)
{
...
HRESULT CTerrain_Desert4::Render()
{
...
void CTerrain_Desert4::Change_Height_Rect(_vector vPickingPos, _float fHeight, _float fRadius)
{
...
void CTerrain_Desert4::Change_Height_Flat(_vector vPickingPos, _float fHeight, _float fRadius)
{
	if (nullptr == m_pVIBufferCom)
		return;
...
void CTerrain_Desert4::Change_Height_Sculpt(_vector vPickingPos, _float fAmount, _float fRadius, _float fMaxHeight)
{
	if (nullptr == m_pVIBufferCom)
		return;
...
void CTerrain_Desert4::Change_Height_Smooth(_vector vPickingPos, _float fFactor, _float fRadius)
{
	if (nullptr == m_pVIBufferCom)
		return;
...
_float CTerrain_Desert4::Get_Height_In_World_Space(_float fWorldX, _float fWorldZ)
{
	// 1. World -> Local 변환 (Terrain_Desert4은 보통 World 행렬이 Identity이므로 무시될 수 있으나, 정석대로 처리)
...
HRESULT CTerrain_Desert4::Ready_Components()
{
	/* Com_VIBuffer */
	if (FAILED(__super::Add_Component(ENUM_CLASS(LEVEL::DESERT), TEXT("Prototype_Component_VIBuffer_Terrain_Desert4"),
...
HRESULT CTerrain_Desert4::Bind_ShaderResources()
{
...
CTerrain_Desert4* CTerrain_Desert4::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTerrain_Desert4* pInstance = new CTerrain_Desert4(pDevice, pContext);
...
CGameObject* CTerrain_Desert4::Clone(void* pArg)
{
	CTerrain_Desert4* pInstance = new CTerrain_Desert4(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CTerrain_Desert4");
		Safe_Release(pInstance);
	}

...
HRESULT CMonster::Initialize(void* pArg)
{
	...
	GameObjectDesc.fRotationPerSec = XMConvertToRadians(180.f);
	GameObjectDesc.fSpeedPerSec = 10.f;

	if (FAILED(__super::Initialize(&GameObjectDesc)))
		return E_FAIL;

	if (FAILED(Ready_Components(pDesc->szModelTag.c_str())))
		return E_FAIL;

	m_pTransformCom->Set_State(STATE::POSITION, pDesc->TransformMatrix.r[3]);
...
HRESULT CMonster::Ready_Components(const _tchar* pComponentTag)
{
	/* Com_Model */

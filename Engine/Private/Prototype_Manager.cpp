...
HRESULT CPrototype_Manager::Add_SkeletalPrototype(_uint iLevelIndex, ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _wstring& strPrototypeTag, const _char* pModelFilePath, const string& strSkeletalPath, vector<_wstring>& szPartPrototypeTagList, vector<string>& szPartModelFilePathList, _fmatrix PreTransformMatrix)
{
	...
	Add_FbxSkeletalAnimation(iLevelIndex, pDevice, pContext, strPrototypeTag, pModelFilePath, strSkeletalPath, szPartPrototypeTagList, szPartModelFilePathList, PreTransformMatrix);
...
HRESULT CPrototype_Manager::Add_FbxSkeletalAnimation(_uint iLevelIndex, ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _wstring& strPrototypeTag, const _char* pModelFilePath, const string& strSkeletalPath, vector<_wstring>& szPartPrototypeTagList, vector<string>& szPartModelFilePathList, _fmatrix PreTransformMatrix)
{
	// _finddata_t : <io.h>에서 제공하며 파일 정보를 저장하는 구조체
	_finddatai64_t  fd;
	...
	while (iResult != -1)
	{
		...
		if (!isExistAnimation(pModel, szAnimationTag))
		{
			WCHAR* szPrototypeTemporaryTag = nullptr;
...
_bool CPrototype_Manager::isExistAnimation(CModel* pModel, const _char* szAnimationTag)
{
	if (nullptr == szAnimationTag)
		return FALSE;

	for (auto& pAnim : *pModel->Get_Animations())
	{
		if (nullptr != pAnim)
		{
			if (0 == strcmp(pAnim->Get_Name(), szAnimationTag))
				return TRUE;
		}
	}

...
CGlass_Scarlet::CGlass_Scarlet(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CPartObject{ pDevice, pContext }
{
}

CGlass_Scarlet::CGlass_Scarlet(const CGlass_Scarlet& Prototype)
	: CPartObject{ Prototype }
{
}

HRESULT CGlass_Scarlet::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CGlass_Scarlet::Initialize(void* pArg)
{
	GLASS_DESC* pDesc = static_cast<GLASS_DESC*>(pArg);
	...
	m_vRotationQuaternion = { 270.f, 0.f, 0.f };
	...
	m_pTransformCom->Set_State(STATE::POSITION, XMVectorSet(0.01f, 0.0f, 0.f, 1.f));

	return S_OK;
}

void CGlass_Scarlet::Priority_Update(_float fTimeDelta)
{
	...
	m_pTransformCom->Set_State(STATE::POSITION, XMVectorSet(-0.1f, 0.5f, 0.323f, 1.f));

}

void CGlass_Scarlet::Update(_float fTimeDelta)
{
...
void CGlass_Scarlet::Late_Update(_float fTimeDelta)
{
...
HRESULT CGlass_Scarlet::Render_Shadow()
{
...
HRESULT CGlass_Scarlet::Ready_Components()
{
	/* Com_Model */
	if (FAILED(__super::Add_Component(ENUM_CLASS(LEVEL::EDITOR), TEXT("Prototype_Component_Model_Glass_Scarlet"),
...
HRESULT CGlass_Scarlet::Bind_ShaderResources()
{
...
CGlass_Scarlet* CGlass_Scarlet::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CGlass_Scarlet* pInstance = new CGlass_Scarlet(pDevice, pContext);
...
CGameObject* CGlass_Scarlet::Clone(void* pArg)
{
	CGlass_Scarlet* pInstance = new CGlass_Scarlet(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CGlass_Scarlet");
		Safe_Release(pInstance);
	}

...
VS_OUT VS_MAIN(VS_IN In)
{
    ...
    vector vPosition = mul(vector(In.vPosition * 0.001, 1.f), In.TransformMatrix);
    vector vNormal = mul(vector(In.vNormal * 0.001, 0.f), In.TransformMatrix);
    vector vTangent = mul(vector(In.vTangent * 0.001, 0.f), In.TransformMatrix);
    vector vBINormal = mul(vector(In.vBInormal * 0.001, 0.f), In.TransformMatrix);
...
PS_OUT PS_MAIN(PS_IN In)
{
    ...
    Out.vDepth = float4(In.vProjPos.z / In.vProjPos.w, In.vProjPos.w / g_CamFar, 0.0f, 1.0f);

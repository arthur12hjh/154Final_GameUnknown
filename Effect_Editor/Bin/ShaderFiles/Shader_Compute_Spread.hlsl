           ...
           uint GI : SV_GroupIndex)
{
    ...
    if (0 > g_Out[DTid.x].vLifeTime.x)
    {
        ...
        {
            g_Out[DTid.x].vfStart = g_WorldMatrix._41_42_43_44 + Input[DTid.x].vfRoot;
    ...
    if (!bisStart && 0 == g_Out[DTid.x].vLifeTime.x)
    {
        ...
        {
            case 0:{
                    g_Out[DTid.x].vfStart = g_WorldMatrix._41_42_43_44 + vDir * Input[DTid.x].vfRoot;
                    g_Out[DTid.x].vTranslation = g_Out[DTid.x].vfStart;
                    g_Out[DTid.x].vLifeTime.x += vfTimeDelta.x;
                    bisStart = true;
                }
 ...
 + (-2 * pow(t, 3) + 3 * pow(t, 2)) * vfGravity.z
    ...
    g_Out[DTid.x].vTranslation += float4(0,1,0, 0) * fGravity * vfTimeDelta.x;
    ...
    g_Out[DTid.x].vRight = float4(normalize(cross(g_Out[DTid.x].WorldMat._21_22_23, g_Out[DTid.x].vLook.xyz)) * Input[DTid.x].vfSize, 0);

...
PS_WEIGHT_OUT PS_POWER(PS_WEIGHT_IN In)
{
    ...
    float s = sin(radians(In.vLifeTime.x * 720));
    float c = cos(radians(In.vLifeTime.x * 720));
    tex -= float2(0.5, 0.5);
    float2x2 rot = float2x2(c, -s, s, c);
    tex = mul(tex, rot) + float2(0.5, 0.5);
    
    float diffuse = (g_DissolveTexture.Sample(NoneSampler, tex).r) * g_DiffuseTexture.Sample(NoneSampler, DiffuseTexcoord).r;
    vector a = g_DiffuseTexture.Sample(NoneSampler, tex);
    float fMax = max(a.r, max(a.g, a.b));
    Out.vDiffuse = g_vColor;
    float2 texcoord = float2((MaskTexcoord.x * 1.2 - (1.2 - 1) * 0.5) + a.r * (0.5 < MaskTexcoord.x ? 0.2 : -0.2), (MaskTexcoord.y * 1.2 - (1.2 - 1) * 0.5) + a.r * (0.5 < MaskTexcoord.y ? 0.2 : -0.2));
    Out.vDiffuse.a *= g_MaskTexture.Sample(NoneSampler, texcoord).r;
    //Out.vDiffuse = g_DiffuseTexture.Sample(NoneSampler, tex);
    //Out.vDiffuse.a *= 0.3;
    Out.vDiffuse.a *= saturate((In.vLifeTime.y - In.vLifeTime.x) * 1.5);
    //if (0 >= Out.vDiffuse.a || 0.5 < length(float2(0.5, 0.5) - texcoord))
    //    discard;
    //Out.vDiffuse.a = g_DissolveTexture.Sample(NoneSampler, tex).r;
    float linearDepth = 0.1 * 500 / (500.f - (In.vProjPos.z / In.vProjPos.w) * (500 - 0.1));
    float weight = saturate(pow(1 - linearDepth / 500, 3));
    Out.vDiffuse.rgb = Out.vDiffuse.rgb * Out.vDiffuse.a * weight;
    Out.vWeight.r = Out.vDiffuse.a * weight;

...
[maxvertexcount(6)]
void GS_NORMAL_STONE(point GS_IN In[1], inout TriangleStream<GS_NORMAL_OUT> OutStream)
{
    GS_NORMAL_OUT Out[4];
...
[maxvertexcount(6)]
void GS_WEIGHT_START(point GS_IN In[1], inout TriangleStream<GS_WEIGHT_OUT> OutStream)
{
    GS_WEIGHT_OUT Out[4];
    ...
    else
    {
        ...
        float3 vNormalRight = normalize(In[0].TransformMatrix._11_12_13);
...
/* «»ºø Ω¶¿Ã¥ı : «»ºø¿« √÷¡æ¿˚¿Œ ªˆ¿ª ∞·¡§«œ≥Æ. */
PS_NORMAL_OUT PS_STONE(PS_NORMAL_IN In, bool isFrontFace : SV_IsFrontFace)
{
    PS_NORMAL_OUT Out;
...
PS_WEIGHT_OUT PS_BLOOD(PS_WEIGHT_IN In, bool isFrontFace : SV_IsFrontFace)
{
    ...
    float2 distort = (g_DiffuseTexture.Sample(MirrorSampler, DiffuseTexcoord).rb * 2.0 - 1.0) * 0.1;
...
/* «»ºø Ω¶¿Ã¥ı : «»ºø¿« √÷¡æ¿˚¿Œ ªˆ¿ª ∞·¡§«œ≥Æ. */
PS_WEIGHT_OUT PS_PIXEL(PS_WEIGHT_IN In)
{
    PS_WEIGHT_OUT Out;
...
/* «»ºø Ω¶¿Ã¥ı : «»ºø¿« √÷¡æ¿˚¿Œ ªˆ¿ª ∞·¡§«œ≥Æ. */
PS_WEIGHT_OUT PS_ELECTRIC3(PS_WEIGHT_IN In)
{
    PS_WEIGHT_OUT Out;
...
/* «»ºø Ω¶¿Ã¥ı : «»ºø¿« √÷¡æ¿˚¿Œ ªˆ¿ª ∞·¡§«œ≥Æ. */
PS_WEIGHT_OUT PS_ELECTRIC4(PS_WEIGHT_IN In)
{
    PS_WEIGHT_OUT Out;
...
/* «»ºø Ω¶¿Ã¥ı : «»ºø¿« √÷¡æ¿˚¿Œ ªˆ¿ª ∞·¡§«œ≥Æ. */
PS_WEIGHT_OUT PS_SMOKEDISTORTION(PS_WEIGHT_IN In)
{
    PS_WEIGHT_OUT Out;
    ...
    float2 uvDistorted = fTexcoord + distort;
    ...
    Out.vDiffuse = g_vColor;
    Out.vDiffuse.a *= min(g_MaskTexture.Sample(DefaultSampler, uvDistorted).r, g_MaskTexture.Sample(DefaultSampler, uvDistorted).a) * saturate(In.vLifeTime.y - In.vLifeTime.x);
...
technique11 DefaultTechnique
{
    ...
    pass Electric4
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = compile gs_5_0 GS_WEIGHT_START();
        PixelShader = compile ps_5_0 PS_ELECTRIC3();
    }
    // idx 16
    pass Electric5
    {
    ...
    pass SmokeDistortion
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = compile gs_5_0 GS_WEIGHT_BILLBOARD();
        PixelShader = compile ps_5_0 PS_SMOKEDISTORTION();
    }
    // idx 20
    pass DSSDistortion
    {

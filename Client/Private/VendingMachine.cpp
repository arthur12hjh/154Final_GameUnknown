...
CVendingMachine::CVendingMachine(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CProb_Interaction{ pDevice, pContext }
{
}

CVendingMachine::CVendingMachine(const CVendingMachine& Prototype)
	: CProb_Interaction{ Prototype }
{
}

HRESULT CVendingMachine::Initialize_Prototype()
{
	return S_OK;
}
...
HRESULT CVendingMachine::Initialize(void* pArg)
{
	...
	m_iObjectID = Object_Number(pDesc->szVIBuffer_PrototypeName);

	return S_OK;
}

void CVendingMachine::Priority_Update(_float fTimeDelta)
{
}

void CVendingMachine::Update(_float fTimeDelta)
{
...
void CVendingMachine::Late_Update(_float fTimeDelta)
{
...
HRESULT CVendingMachine::Ready_Components(const _tchar* pComponentTag)
{
	/* Com_Model */
...
HRESULT CVendingMachine::Bind_ShaderResources()
{
...
void CVendingMachine::SetCullingCollider(_uint iObjectID)
{
...
_uint CVendingMachine::Object_Number(const _tchar* pComponentTag)
{
...
HRESULT CVendingMachine::Begin_OverlapCallBack()
{
	return S_OK;
}

HRESULT CVendingMachine::End_OverlapCallBack()
{
	return S_OK;
}

void CVendingMachine::Excute_CallBack(CGameObject* pActionObject)
{
}

CVendingMachine* CVendingMachine::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CVendingMachine* pInstance = new CVendingMachine(pDevice, pContext);
...
CGameObject* CVendingMachine::Clone(void* pArg)
{
	CVendingMachine* pInstance = new CVendingMachine(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CVendingMachine");
		Safe_Release(pInstance);
	}

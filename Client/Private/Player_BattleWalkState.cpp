...
void CPlayer_BattleWalkState::Start(void* pArg, _float fBlendRatio)
{
    ...
    else if (true == m_isEvading && true == m_isLanding)
    {
...
PLAYER_TRANSITION_DESC CPlayer_BattleWalkState::Update(_float fTimeDelta)
{
    ...
    if (true == m_isTurning && (33.f / 36.f) <= fAnimationRatio)
    {
    ...
    if (true == m_isTurning && (12.f / 36.f) <= fAnimationRatio)
        m_isMovable = true;
    ...
    // Turn 트리거 (입력 있을 때만 트리거)
    if (false == m_isTurning && false == m_isTurnLocked && false == m_isRunStart && false == m_isLanding &&
        false == m_isEvading && true == isWalking && true == isInput && fDot <= fTurnDotThreshold)
    {
        m_pPlayer->Set_Animation("Proto_Battle_Run_Turn_L", false, 1.5f, 0.f, false, -1.f, 2.5f, true);
        m_isTurning = true;
        m_isTurnLocked = true;
        m_isMovable = false;
    }
    ...
    if (false == m_isTurning || true == m_isMovable)
        m_Desc->pPlayerTransform->Go_Straight(fTimeDelta * 1.65f);

...
CBullet_Split::CBullet_Split(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) :
    CBullet(pDevice, pContext)
{
}

CBullet_Split::CBullet_Split(const CBullet_Split& Prototype) :
    CBullet(Prototype)
{
}

HRESULT CBullet_Split::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CBullet_Split::Initialize(void* pArg)
{
...
void CBullet_Split::Priority_Update(_float fTimeDelta)
{
}

void CBullet_Split::Update(_float fTimeDelta)
{
...
void CBullet_Split::Late_Update(_float fTimeDelta)
{
...
void CBullet_Split::Shoot_Projectile(_vector vTargetPoint, _float fSpeed)
{
	Update_BulletCombinedMatrix();
...
HRESULT CBullet_Split::ADD_Components(BULLET_DESC& pDesc)
{
...
void CBullet_Split::Begin_OverlapEvent(_float3 vHitPoint, _float3 vHitDir, CGameObject* pHitActor)
{
	// 여기서 일단 데미지 처리한다
// 맞으면 맞는 모션 나오게
...
CBullet_Split* CBullet_Split::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBullet_Split* pBullet_Split = new CBullet_Split(pDevice, pContext);
	if (FAILED(pBullet_Split->Initialize_Prototype()))
	{
		Safe_Release(pBullet_Split);
		MSG_BOX("Create Fail : Bullet Split");
	}
    return pBullet_Split;
}

CGameObject* CBullet_Split::Clone(void* pArg)
{
	CBullet_Split* pBullet_Split = new CBullet_Split(*this);
	if (FAILED(pBullet_Split->Initialize(pArg)))
	{
		Safe_Release(pBullet_Split);
		MSG_BOX("Clone Fail : Bullet Split");
	}
	return pBullet_Split;
}

void CBullet_Split::Free()
{
	__super::Free();
}

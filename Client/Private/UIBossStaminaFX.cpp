...
CUIBossStaminaFX::CUIBossStaminaFX(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIBase{ pDevice, pContext }
{
}

CUIBossStaminaFX::CUIBossStaminaFX(const CUIBossStaminaFX& Prototype) 
	: CUIBase{ Prototype }
{
}

HRESULT CUIBossStaminaFX::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CUIBossStaminaFX::Initialize(void* pArg)
{	
...
void CUIBossStaminaFX::Priority_Update(_float fTimeDelta)
{
	__super::Priority_Update(fTimeDelta);
}

void CUIBossStaminaFX::Update(_float fTimeDelta)
{
	__super::Update(fTimeDelta);

	if (static_cast<CUIBossStamina*>(m_pParent))
		m_iCurrentStamina = static_cast<CUIBossStamina*>(m_pParent)->Get_CurrentStamina();
	
	auto pStamina = static_cast<CUIBossStamina*>(m_pParent);
	if (!pStamina) return;

	_float fill = pStamina->Get_CurrentFill() - ((3.f / pStamina->Get_UIBase_Desc().fSizeX) * 2.f);
	...
	if (m_iCurrentStamina != m_iPrevStamina && m_iCurrentStamina < m_iPrevStamina)
	{
		...
		auto AnimTag = m_tUIDesc.m_AnimTags.find(TEXT("Boss_Stamina_FX"));
...
void CUIBossStaminaFX::Late_Update(_float fTimeDelta)
{
...
HRESULT CUIBossStaminaFX::Render()
{
	__super::Render();
...
HRESULT CUIBossStaminaFX::Ready_Components()
{
...
HRESULT CUIBossStaminaFX::Bind_ShaderResources()
{
...
HRESULT CUIBossStaminaFX::Execute(const UI_EVENT_DESC& EventDesc)
{
	return S_OK;
}

void CUIBossStaminaFX::CallbackEvent(void* pArg)
{
...
CUIBossStaminaFX* CUIBossStaminaFX::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CUIBossStaminaFX* pInstance = new CUIBossStaminaFX(pDevice, pContext);
...
CGameObject* CUIBossStaminaFX::Clone(void* pArg)
{
	CUIBossStaminaFX* pInstance = new CUIBossStaminaFX(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CUIBossStaminaFX");
		Safe_Release(pInstance);
	}

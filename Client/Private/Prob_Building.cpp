#include "pch.h"
#include "Prob_Building.h"

#include "GameInstance.h"

CProb_Building::CProb_Building(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) :
    CActor(pDevice, pContext)
{
}

CProb_Building::CProb_Building(const CProb_Building& Prototype) :
    CActor(Prototype)
{
}

HRESULT CProb_Building::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CProb_Building::Initialize(void* pArg)
{
...
void CProb_Building::Priority_Update(_float fTimeDelta)
{
}

void CProb_Building::Update(_float fTimeDelta)
{
...
void CProb_Building::Late_Update(_float fTimeDelta)
{
...
HRESULT CProb_Building::ADD_Components(const ACTOR_DESC& Desc)
{
    /* Com_Model */
...
HRESULT CProb_Building::Bind_ShaderResources()
{
...
CProb_Building* CProb_Building::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CProb_Building* pProb_Building = new CProb_Building(pDevice, pContext);
    if (FAILED(pProb_Building->Initialize_Prototype()))
    {
        Safe_Release(pProb_Building);
        MSG_BOX("Create Fail : Prob Building");
    }
    return pProb_Building;
}

CGameObject* CProb_Building::Clone(void* pArg)
{
    CProb_Building* pProb_Building = new CProb_Building(*this);
    if (FAILED(pProb_Building->Initialize(pArg)))
    {
        Safe_Release(pProb_Building);
        MSG_BOX("Clone Fail : Prob Building");
    }
    return pProb_Building;
}

void CProb_Building::Free()
{
    __super::Free();

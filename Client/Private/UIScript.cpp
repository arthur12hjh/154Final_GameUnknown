...
void CUIScript::Late_Update(_float fTimeDelta)
{
	...
	if (m_fTimeAcc >= 10.f)
	{
		...
		if (m_iScriptIdx >= m_Scripts.size())
			m_iScriptIdx = 0;
	}
}
...
HRESULT CUIScript::RenderText()
{
	_float2 fTextSize = m_pGameInstance->Get_Text_Size(
		m_tUIDesc.m_tUITextDesc.szFont.c_str(), m_Scripts[m_iScriptIdx].szScriptText.c_str(), true, m_tUIDesc.m_tUITextDesc.fScale);
	_float fAlpha{ m_tUIDesc.fAlpha };

	_float2 vPivot{
		m_tUIDesc.fX + m_tUIDesc.fOffsetX - (fTextSize.x * 0.5f), //- (m_tUIDesc.fSizeX * 0.5f) + (fTextSize.x * 0.5f),
		m_tUIDesc.fY + m_tUIDesc.fOffsetY //- (m_tUIDesc.fSizeY * 0.5f) + (fTextSize.y * 0.5f)
	};
	...
	_vector vColor = XMVectorSet(
		m_Scripts[m_iScriptIdx].vColor.x * fAlpha,
		m_Scripts[m_iScriptIdx].vColor.y * fAlpha,
		m_Scripts[m_iScriptIdx].vColor.z * fAlpha,
		m_Scripts[m_iScriptIdx].vColor.w * fAlpha
	);
	...
	m_pGameInstance->Render_Text(m_tUIDesc.m_tUITextDesc.szFont.c_str(),
		m_Scripts[m_iScriptIdx].szScriptText.c_str(),
		vShadowPivot,

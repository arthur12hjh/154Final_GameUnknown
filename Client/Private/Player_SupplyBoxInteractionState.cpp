...
void CPlayer_SupplyBoxInteractionState::Start(void* pArg, _float fBlendRatio)
{
    ...
    m_pPlayer->GetTransform()->Set_State(STATE::POSITION, XMVector3Normalize(XMVectorSetY(vTargetLook, 0.f)) * 3.6f + XMVectorSetY(vTargetPos, XMVectorGetY(vPlayerPos)));
    m_Desc->pPlayerController->Set_Position(XMVectorSetY(vTargetLook, 0.f) * 3.68f + XMVectorSetY(vTargetPos, XMVectorGetY(vPlayerPos)));
}

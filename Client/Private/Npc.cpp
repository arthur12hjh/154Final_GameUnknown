...
void CNpc::Npc_Action()
{
    ...
    case NPC_STATE::TALK:
    {
        if (!pScript->Get_Has_Script_Desc())
        {
            ...
            if (m_NpcDesc->iNpcID == ENUM_CLASS(NPC_ID::DORORONG)) // 상점 npc || // 도로롱 npc
            {
                if (m_iScriptIdx == 0)
                    m_eNpcState = NPC_STATE::ACTION_BEGIN;
                else if (m_iScriptIdx > 0)
    ...
    {
        if (m_NpcDesc->iNpcID == ENUM_CLASS(NPC_ID::DIG)) // 상점 npc
        {
            pHUD->Open_Shop();
            m_eNpcState = NPC_STATE::ACTION;
        }

        if (m_NpcDesc->iNpcID == ENUM_CLASS(NPC_ID::DORORONG) && m_iScriptIdx == 0) // 도로롱 npc
        {
    ...
    case NPC_STATE::BYE:
    {
        if (m_NpcDesc->iNpcID == ENUM_CLASS(NPC_ID::DORORONG)) // 도로롱 npc
        {

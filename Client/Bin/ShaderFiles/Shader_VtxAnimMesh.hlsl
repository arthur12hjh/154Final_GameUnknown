...
VS_OUT VS_MAIN(VS_IN In)
{
    ...
    In.vBlendWeight = (In.vBlendWeight < 0.05f) ? 0.0f : In.vBlendWeight;

    float fWeightSum = In.vBlendWeight.x + In.vBlendWeight.y + In.vBlendWeight.z + In.vBlendWeight.w;
    if (fWeightSum > 1e-4f)
    {
        In.vBlendWeight /= fWeightSum;
    }
    else
    {
        In.vBlendWeight = float4(1, 0, 0, 0);
    }
...
technique11 DefaultTechnique
{
    ...
    pass Dororong
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_DORORONG();
    }
}

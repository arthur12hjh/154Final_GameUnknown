           ...
           uint GI : SV_GroupIndex)
{
    ...
    if (0 > g_Out[DTid.x].vLifeTime.x)
    {
        ...
        else if(1 == vfisSphere.x)
        {
            ...
            g_Out[DTid.x].vfRoot = g_WorldMatrix._41_42_43_44 + vDir * vfisSphere.y;
        ...
        else if (2 == vfisSphere.x)
        {
            if (1 == viLoopAndCount.x)
            {
                ...
                g_Out[DTid.x].vfRoot = g_WorldMatrix._41_42_43_44 + Input[DTid.x].vfRoot + vRight * vfisSphere.y + vUp * vfisSphere.y;
                ...
                vDir = normalize(float4(Input[DTid.x].vTranslation.xyz - g_Out[DTid.x].vfRoot.xyz, 0));
            }
            else
            {
                g_Out[DTid.x].vfRoot = Input[DTid.x].vfRoot;
    ...
    else if (2 == vfisSphere.x && 0 == g_Out[DTid.x].vLifeTime.x)
    {
        if (0 == viLoopAndCount.x)
        {
            if (!bisStart)
            {
                float4 vRight = float4(float3(1, 0, 0) * sin(float(DTid.x) / viLoopAndCount.y * radians(vfCircle.x) - vfTimeDelta.x), 0);
                float4 vUp = float4(float3(0, 0, 1) * cos(float(DTid.x) / viLoopAndCount.y * radians(vfCircle.x) - vfTimeDelta.x), 0);
                g_Out[DTid.x].vfRoot = g_WorldMatrix._41_42_43_44 + Input[DTid.x].vfRoot + vRight * vfisSphere.y + vUp * vfisSphere.y;
                vRight = float4(float3(1, 0, 0) * sin(float(DTid.x) / viLoopAndCount.y * radians(vfCircle.x)), 0);
                vUp = float4(float3(0, 0, 1) * cos(float(DTid.x) / viLoopAndCount.y * radians(vfCircle.x)), 0);
                g_Out[DTid.x].vTranslation = g_WorldMatrix._41_42_43_44 + Input[DTid.x].vfRoot + vRight * vfisSphere.y + vUp * vfisSphere.y;
 ...
 + (-2 * pow(t, 3) + 3 * pow(t, 2)) * vfGravity.z
    ...
    if (1 == viLoopAndCount.x && g_Out[DTid.x].vLifeTime.x >= g_Out[DTid.x].vLifeTime.y)
    {
        if (2 == vfisSphere.x)
        {
            ...
            vRight = float4(float3(1, 0, 0) * sin(vfTimeDelta.y), 0);
            vUp = float4(float3(0, 0, 1) * cos(vfTimeDelta.y), 0);
            g_Out[DTid.x].vTranslation = g_WorldMatrix._41_42_43_44 + vRight * vfisSphere.y + vUp * vfisSphere.y;
           ...
           uint GI : SV_GroupIndex)
{
    ...
    if (0 > g_Out[DTid.x].vLifeTime.x)
    {
        ...
        else if (2 == vfisSphere.x)
        {
            ...
            float4 vDir = normalize(float4(g_Out[DTid.x].vfRoot.xyz - vfPivot.xyz, 0));
 ...
 + (-2 * pow(t, 3) + 3 * pow(t, 2)) * vfGravity.z
    ...
    if (0.01 > length(g_Out[DTid.x].vTranslation - Input[DTid.x].vTranslation))
        g_Out[DTid.x].vLook = Input[DTid.x].vLook;
    else
        g_Out[DTid.x].vLook = normalize(g_Out[DTid.x].vTranslation - Input[DTid.x].vTranslation);
    
    g_Out[DTid.x].vRight = float4(normalize(cross(float3(0.f, 1.f, 0.f), g_Out[DTid.x].vLook.xyz)), 0);
    g_Out[DTid.x].vUp = float4(normalize(cross(g_Out[DTid.x].vLook.xyz, g_Out[DTid.x].vRight.xyz)), 0);
    if (!bisStart)
    {
        g_Out[DTid.x].vLifeTime.x += vfTimeDelta.x;
    }

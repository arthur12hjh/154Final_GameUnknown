...
PS_OUT_GLOW_FINAL PS_MAIN_GLOW_FINAL(PS_IN In)
{
    ...
    for (int i = -10; i < 11; ++i)
    {
        ...
        vColor += g_fBloomWeights[i + 10] * g_GlowTexture.Sample(ClampSampler, vTexcoord) * 100;
        vWeight += g_fBloomWeights[i + 10] * g_WeightTexture.Sample(ClampSampler, vTexcoord) * 100;
    }
    vColor /= 6.5f;
